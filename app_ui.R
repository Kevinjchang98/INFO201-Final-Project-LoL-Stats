ui <- material_page(title = "LoL Stats",
   ...
   material_side_nav(
     ...
     fluidRow(
       ...
       column(
         10,
         p("Created by Kevin Chang for the final project of INFO 201 with Prof. T. Winegarden. Built with R and Shiny with plots in Plotly. Layout uses shinymaterial, found ", a("here", href = "https://ericrayanderson.github.io/shinymaterial/")),
   ...
   material_tab_content(tab_id = "tableStats",
            ...
            fluidRow(
               ...
               column(10,
                  ...
                  p("This table provides insight for anyone wanting to know very specific values, or sort by specific values. For example, if one wants to see quickly what their best champion in terms of average CS was in all the games they won recently, or if one wants to see the champion with the worst KDA in games they lost, this table provides a very easy way to sort and get a list. For this sort of data analysis, a table was chosen as being more effective than a graph, as the purpose is to allow one to get a ranking in terms of various factors.")
               ),
               column(1)
            )
   ),
   ...
   material_tab_content(tab_id = "champGraph",
            ...
            fluidRow(
               ...
               column(10,
                      h4("INFO 201 Information"),
                      p("Here, a bar chart further split into win/loss by color allows players to see various information about the champions they played recently. One can see how many different champions they played with the number of elements on the x-axis, as well as which are more popular than others with their values on the y-axis. One can also get a quick overview at which champions may be more effective, as they can see the rough winrates with the colored splitting of the bar graph.")
               ),
               column(1)
            )
   ),
   ...
   material_tab_content(tab_id = "timeGraph",
            ...
            fluidRow(
               ...
               column(10,
                      h4("INFO 201 Information"),
                      p("Here, a bar graph similar to the champions tab was chosen for similar reasons. As the data is binned by hour start, a line graph is not appropriate, and the splitting of the bar graph into win and loss allows for analysis of whether there's an optimal time to play for the specific player.")
               ),
               column(1)
            )
   ),
   ...
   material_tab_content(tab_id = "winLossGraph",
      ...
      fluidRow(
         ...
         column(10,
                h4("INFO 201 Information"),
                p("Here, four key statistics are averaged over the games lost and won, and compared. This allows for investigating which factors may possibly be a cause for winning or losing. Possible conclusions are elaborated upon further in the summary section.")
         ),
         column(1)
      )
   ),
   ...
   material_tab_content(tab_id = "timeStatsGraph",
      ...
      fluidRow(
         ...
         column(10,
                h4("INFO 201 Information"),
                p("Here, a scatter plot was chosen to allow for analysis of any patterns over time. With a linear plotting of game number and a loess curve fit, one can quickly see if there appears to be any trends in a variety of variables that can be plotted on the y-axis. One may also choose to plot the games by their date, which may be more appropriate for players that may have large gaps in play time, as that also allows them to analyze any overarching patterns in their clusters of games as opposed to the linear plotting with game number.")
         ),
         column(1)
      )
   ),
